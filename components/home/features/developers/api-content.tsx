    ...
    return (
        <div className="flex flex-col justify-center">
            ...
            <div className="mt-6 flex space-x-6 items-center uppercase">
                ...
                >
                    {isAuthenticated ? (
                        <Link href={APP_ROUTES.API.MANAGEMENT}>Go to API Keys</Link>
                    ) : (
                        <Link href={APP_ROUTES.API.MANAGEMENT}>Get Started Free</Link>
                    )}
                </Button>
                <Link href={APP_ROUTES.API.DOCS} className="font-bold text-sm tracking-wide">
                    Documents
                </Link>
            </div>
        </div>
